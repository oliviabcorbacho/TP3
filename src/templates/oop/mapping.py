#from curses import keyname this is the right one xx
import random
...
class Level:
    ...
    def render(self, player: player.Player, gnome: gnome.Gnome, current_level:int, gnome2: gnome.Gnome):
        ...
        for i, row in enumerate(self.tiles):
            ...
            print("|")
        print("-" + "-" * len(self.tiles[0]) + "-")
        print('PLAYER: {}'.format(player.name), '\t HP: {}'.format(player.hp), '\t WEAPON: {}'.format(player.weapon),'\t TOOL: {}'.format(player.tool) ,'\t LOCATION: ({}, {})'.format(player.loc()[0], player.loc()[1]))
        print(f"Collect the weapons to reach the treassure! Your enemy is the gnome and has {gnome.hp} hit points.")
        print(f'Watch out! On level two is the second gnome! He has {gnome2.hp} hit points.')
        if player.hp <= 10:
    ...
    def are_connected(self, initial: Location, end: Location) -> bool:
        ...
        for position in consecutives:
            if 0 <= position[0] <= self.columns and 0 <= position[1] <= self.rows and self.is_walkable(position) and position not in self.visited:
